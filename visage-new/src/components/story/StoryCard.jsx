...
const StoryCard = ({ story, onLike, onUnlike, onComment, currentUser }) => {
    ...
    // Add defensive check for story.likes to ensure it exists and is an array
    const isLiked =
        Array.isArray(story?.likes) && currentUser
    ...
    const handleCommentSubmit = async (e) => {
        e.preventDefault();
        if (!commentText.trim() || !story || !currentUser) return;
    ...
    // Safety check - if story is undefined or null, don't render anything
    if (!story) {
        return null;
    }

    // Ensure all story properties exist with defaults to prevent errors
    const {
        _id = "",
    ...
    return (
        <StoryCardContainer type={cardType} hasMedia={hasMedia}>
            <CardHeader>
                <UserInfo>
                    ...
                    <UserDetails>
                        ...
                        <StoryType>
                            {storyType === "poetry" ? "Poetry" : "Thought"}
                        </StoryType>
                    </UserDetails>
                </UserInfo>
            ...
            <CardContent to={`/s/${_id}`} type={storyType} hasMedia={hasMedia}>
                {mediaType === "image" && mediaUrl ? (
                    <StoryImage src={mediaUrl} alt="Story" />
            ...
            <CardActions>
                <ActionButton onClick={handleLikeToggle}>
                    ...
                    <ActionCount>{likes.length}</ActionCount>
                </ActionButton>
